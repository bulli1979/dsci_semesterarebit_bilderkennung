...
try:
    ...
    try:
        ...
        USE_SKIMAGE_HSV = True
    except ImportError:
        from matplotlib.colors import rgb_to_hsv
        USE_SKIMAGE_HSV = False
...
def generate_mask_with_hsv_threshold(im, sign=None,hue_threshold=None, saturation_threshold=None, value_threshold=None):
    ...
    # Stelle sicher, dass das Array kopierbar ist
    if not isinstance(im, np.ndarray):
        im = np.array(im)
    
    # Konvertiere zu float32 und normalisiere auf 0-1 Bereich
    # WICHTIG: Verwende .copy() um sicherzustellen, dass es eine echte Kopie ist
    if im.dtype == np.uint8:
        # Explizite Kopie als float32, normalisiert auf 0-1
        im_normalized = (im.astype(np.float32) / 255.0).copy()
    else:
        # Für andere dtypes, konvertiere zu float32
        im_normalized = im.astype(np.float32).copy()
        if im_normalized.max() > 1.0:
            im_normalized = (im_normalized / 255.0).copy()
    
    # Stelle sicher, dass das Array kontinuierlich ist
    im_normalized = np.ascontiguousarray(im_normalized)
    
    # Eigene RGB-zu-HSV-Konvertierung (umgeht matplotlib copy=False Problem komplett)
    # Basierend auf der Standard-RGB-zu-HSV-Formel
    r, g, b = im_normalized[:,:,0], im_normalized[:,:,1], im_normalized[:,:,2]
    
    # Finde Maximum und Minimum für jeden Pixel
    max_val = np.maximum(np.maximum(r, g), b)
    min_val = np.minimum(np.minimum(r, g), b)
    delta = max_val - min_val
    
    # Value (Helligkeit) = Maximum
    v = max_val.copy()
    ...
    # Grün ist Maximum
    mask_g = (max_val == g) & (delta > 0) & ~mask_r
    h[mask_g] = (60.0 * ((b[mask_g] - r[mask_g]) / delta[mask_g]) + 120.0) % 360.0
    
    # Blau ist Maximum
    mask_b = (max_val == b) & (delta > 0) & ~mask_r & ~mask_g
    h[mask_b] = (60.0 * ((r[mask_b] - g[mask_b]) / delta[mask_b]) + 240.0) % 360.0
    
    # Kombiniere zu HSV-Array und skaliere auf 0-255 für alle Komponenten
    # (entspricht der ursprünglichen Implementierung mit [255, 255, 1])
    im_hsv = np.stack([
        h / 360.0 * 255.0,  # Hue: 0-360 -> 0-255
        s * 255.0,          # Saturation: 0-1 -> 0-255
        v * 255.0           # Value: 0-1 -> 0-255 (korrigiert!)
    ], axis=-1) 
